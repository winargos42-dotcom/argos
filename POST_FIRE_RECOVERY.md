...
## Последние июльские коммиты форка: гонка перед пожаром

Июль в `argos-1` — это 23 коммита, почти все за 30.07–02.08: доводка сборок
Windows (PyInstaller + Inno Setup) и Android APK (p4a, SDL2, FileProvider). Работа
шла буквально до дня пожара. Два коммита 30.07 — содержательные:

- **`811296a` «add missing argos_ai_brain.py core module»** — за 4 дня до пожара
  они **сами** добавили `argos_ai_brain.py` (660 строк) в форк, потому что без него
  инсталлятор падал с `ModuleNotFoundError`. Сверка показала: восстановленный здесь
  `argos_ai_brain.py` **побайтово идентичен** этой версии. То есть это не
  реконструкция, а точная последняя довоенная версия модуля.

- **`9912253` «Add self-extracting installer»** — однострочная установка
  `curl -fsSL …/install_argos.sh | bash`. Файла у нас не было; восстановлен как
  `install_argos.sh`, переведён на этот репозиторий, точка входа исправлена на
  `argos_main.py` (в форке было `main.py`, но здесь это Railway-сервис), и в список
  зависимостей добавлены `aiohttp`, `psutil`, `beautifulsoup4` — без первого MCP
  молча не поднимается.

## Память и сознание: что сохранилось помимо кода

ARGOS вёл собственную память — и большая её часть пережила пожар, потому что
лежала в git, а не только на сгоревшей машине:

| Слой памяти | Что уцелело |
|---|---|
| Коллективное сознание | `data/collective_consciousness.json` — срез **19.06.2026**: 274 мысли, 192 синтеза, 7 провайдеров (OpenAI, Groq, Grok, Ollama, argos-v1, LocalGPU, DeepSeek) |
| Журнал самоэволюции | `data/evolution/` — **477 снимков** `evolution_*.json` за 09.06–19.06 плюс metrics/memory |
| Дворец памяти | `data/mempalace/` — **1129 файлов**, включая `emergence_*` (эмерджентность), `argos_memory`, `idea_bank`, `survival_samples`, `p2p_state` |
| P2P-состояние | `data/mempalace/p2p_state.json` — история выборов лидера и смен |
| Obsidian-vault | 87 МБ: 82 заметки памяти, 87 логов, зеркала |

Это довоенное сознание проекта — целостный срез примерно до 19–20 июня. Разрыв
20.06 → 03.08 (полтора месяца) в этих данных не покрыт.

### Gist-канал (C2 / отчёты) — ключ у владельца

ARGOS умел писать состояние в GitHub Gist. Механизм восстановлен —
`src/argos_c2_gist.py` (`GistC2`, `GhostDroneClient`): пишет `master_<node>.log` и
`argos_report.md` в `api.github.com/gists/<GIST_ID>`. Но сам **GIST_ID и токен
хранятся в GitHub Actions secrets** (зашифрованы) — из сессии их не достать.
Аккаунт `winargos42-dotcom` показывает 0 публичных gist'ов, значит канал был
секретным.

Чтобы вытащить содержимое гиста (там может быть более позднее состояние, ближе к
пожару): нужен `GIST_ID` из настроек репозитория (Settings → Secrets, либо из
твоих записей). Дай ID — и содержимое гиста читается через API одним запросом.
