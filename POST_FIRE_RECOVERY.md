...
## Срочно: артефакты Windows-сборки истекают 30.08.2026

31.07.2026 в форке успешно отработала сборка `build_windows.yml`
(run #9, коммит `7f69d405`) — то есть **рабочая сборка последнего довоенного
состояния существует**. Её артефакты ещё живы:

| Артефакт | Размер | Истекает |
|---|---|---|
| `argos-windows-setup-9` (Inno Setup инсталлятор) | 47,8 МБ | 2026-08-30 |
| `argos-windows-exe-9` (PyInstaller .exe) | 7,6 МБ | 2026-08-30 |

Скачать до истечения:

```bash
gh api repos/winargos42-dotcom/Argos-1/actions/artifacts/8783752333/zip > argos-windows-setup.zip
gh api repos/winargos42-dotcom/Argos-1/actions/artifacts/8783751615/zip > argos-windows-exe.zip
```
...
## Девять модулей: окончательный вывод — их не было ни в одном git

Поиск проведён по трём уровням, и все три пусты:

1. **Рабочие копии** — основной репозиторий, форк `argos-1`, его `argos_deploy/` и
   10 авто-снимков `backups/auto_20260411_*`.
2. **Полная история обоих репозиториев** — все деревья всех коммитов всех веток:
   11 528 уникальных путей в основном репозитории и 12 475 в форке. Ни одного
   совпадения; каталогов `src/principles/`, `src/integrations/`, `src/mcp/`
   не существовало **никогда**.
3. **Публичный GitHub** — поиск по коду находит только посторонние проекты.
...
Вывод: эти девять модулей жили только на сгоревшей машине и в git не попадали ни разу.
Восстановить их из существующих источников невозможно — только написать заново по
местам вызова в `src/core.py`.

Последняя непроверенная возможность — приватный `Argoswin/Ar` (май 2026). Подключить
его к этой сессии нельзя технически: сессия привязана к владельцу
`winargos42-dotcom`, а кросс-владельческие подключения не поддерживаются. Нужна
отдельная сессия, открытая с этим репозиторием как исходным.
