...
SKILL_DESCRIPTION = "Сканирование локальной сети: хосты, порты, сервисы"
...
class NetGhost:
    ...
    def ping_scan(self, subnet: str = "") -> list[str]:
        ...
        if not subnet:
            # Используем NETSCAN_DEFAULT_RANGE из .env, если задан (напр. "192.168.1.0/24")
            env_range = os.getenv("NETSCAN_DEFAULT_RANGE", "").strip()
            if env_range:
                subnet = ".".join(env_range.split("/")[0].split(".")[:3])  # "192.168.1.0/24" → "192.168.1"
            else:
