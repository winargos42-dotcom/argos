...
class EditorStepDiagnosticBrowser:
    def __init__(self):
    ...
    async def inspect(self):
        ...
        if self.inspect_calls == 1:
            return BrowserResult(status="ui_changed", reason="step-specific editor")
...
@pytest.mark.asyncio
async def test_live_bridge_does_not_redirect_safe_campaign_editor_step(tmp_path):
    draft_url = "https://planeta.ru/campaigns/251138/edit/about"
    config = PlanetaConfig(
        base_url="https://planeta.ru",
        draft_url=draft_url,
